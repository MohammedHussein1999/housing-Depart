...
function updateCities() {
    ...
    cities.innerHTML = '<option value="" selected disabled>اختر المدينة</option>';

    if (regionsVal) {
        const cityFiltering = citiesArray.filter(city => city.region_id == regionsVal);
        cityFiltering.forEach(city => {
            let newOption = document.createElement("option");
            newOption.value = city.city_id;  // Use name_en or name_ar based on your preference
            newOption.text = city.name_ar;
            cities.add(newOption);
        });
    }
}
function updateCitiesTwo() {
    const cities = document.getElementById("citiesTwo");
    const regions = document.getElementById("regionsTwo");
