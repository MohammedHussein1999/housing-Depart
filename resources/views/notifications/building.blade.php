                ...
                <div class="col-md-12 mb-3">
                    <div class="card card-plain ">
                        <div class="card-header">
                            <h5 class="card-title text-right">تنبيهات الوحدات @if (Auth::user()->type == 3)
                                    التي تم رفضها
                                @endif
                            </h5>
                        </div>
                                    ...
                                    @foreach ($building as $item)
                                        @if (Auth::user()->type == 1)
                                            <tr>
                                                ...
                                                <td>{{ $item->city }}</td>
                                                <td>
                                                    <a href="{{ route('building.approve', $item->id) }}"
                                                        class="btn btn-success py-0">موافقة</a>
                                                    <a href="{{ route('building.notApprove', $item->id) }}"
                                        ...
                                        @elseif (Auth::user()->type == 3)
                                            <tr>
                                                ...
                                                <td>{{ $item['city'] }}</td>
                                                <td>
                                                    <a href="{{ route('building.again', $item['id']) }}"
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            globalTableOne.forEach((row) => {
                ...
                if (tds.length) {
                    const tdOne2 = tds[1];
                    const idValueOne2 = tdOne2.textContent.trim();

                    const FindRegion2 = coll.find((region) => {
                        return region.id == idValueOne2;
                    });
                    if (FindRegion2) {
                        tdOne2.innerText = FindRegion2.name;
                    }
